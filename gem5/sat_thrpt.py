	...
	for b in range(len(bench)):
		for v in range(len(vc_)):
			...
			while(pkt_lat < 500.00):
				...
				output_dir=("{0:s}/{1:d}/{3:s}/{2:s}/vc-{4:d}/inj-{5:1.2f}".format(out_dir, num_cores[c],  bench_caps[b], routing_algorithm[rout_], vc_[v], injection_rate))
